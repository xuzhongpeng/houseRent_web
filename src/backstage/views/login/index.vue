      ...
      handleLogin() {
        ...
        this.$refs.AccountFrom.validate((valid) => {
          if (valid) {
            ...
            console.log(valid);
            this.getData('administratorsController/getAdmin.do',loginParams,
                function(res){
                    var data=res.data;console.log(data);
                    that.loading = false;
                    if(data.type=='success'){                   
                        that.setItem("userInfo",JSON.stringify(data.result));
                        that.$router.push("/backstage/home");
                    }
                    else{
                     ...
                     that.loading = false;
                    that.$message("内部可能有点问题哦 错误码："+JSON.stringify(error.status));
                })
